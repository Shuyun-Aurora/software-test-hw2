...
TC08 & 移动端 Web UI 综合验证 & Android Chrome 中核心购物流程 & 使用 Appium 驱动移动端 Chrome，验证登录、商品列表、排序、滚动、详情查看、购物车、结算和订单完成流程 & 页面元素可见，触控点击、滚动、表单输入和订单完成流程正常 & 成员 C \\
\bottomrule
\end{longtable}
\endgroup
...
\subsection{TC08 移动端 Web UI 综合验证}
...
\begin{enumerate}
  ...
  \item 使用排序下拉框选择价格升序，检查排序结果。
  \item 滑动商品列表页面，检查商品卡片和按钮在移动端滚动场景下是否可见。
  \item 进入商品详情页，检查商品名称和价格信息。
  \item 返回商品列表页，点击一个商品的 Add to cart 按钮，检查购物车 badge 是否更新为 1。
  \item 进入购物车页，检查商品名称和数量。
  \item 进入结算流程，填写 first name、last name 和 postal code。
  \item 进入订单确认页并点击 Finish，检查订单完成提示。
\end{enumerate}

预期结果：移动端浏览器环境下登录、商品浏览、排序、页面滑动、商品详情查看、加入购物车、购物车检查、结算信息填写和订单完成流程均可正常执行。
